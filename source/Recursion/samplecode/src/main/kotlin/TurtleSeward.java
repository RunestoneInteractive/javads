...
public class TurtleSeward implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{

    private static ArrayList<TurtleSeward> turtles;
    ...
    private static TurtleSeward turtle;
    ...
    private static HashMap<TurtleSeward,ArrayList<ArrayList>> mouseBindings;
    ...
    private static TurtleSeward selectedTurtle;
    ...
    private static void init()
    {
        ...
        turtles=new ArrayList<TurtleSeward>();
        ...
        mouseBindings=new HashMap<TurtleSeward,ArrayList<ArrayList>>();
        centerX=0;
        centerY=0;
        scale=1;
...
//        System.out.println(Arrays.toString(fontNames));
     ...
     */
    public TurtleSeward(double x, double y)
    {
        if(window==null)init();
     ...
     */
    public TurtleSeward clone()
    {
        TurtleSeward t=new TurtleSeward(0);
    ...
    private static void retrieveState(long time)
    {
        synchronized(turtleLock)
        {
            TurtleSeward t=getStateTurtle(turtleStates.get(time));
            t.__time=t._time;
    ...
    private static TurtleSeward getStateTurtle(ArrayList state){return (TurtleSeward)state.get(1);}
    ...
    public long stab()
    {
        Color c= TurtleSeward.getColor("red");
    ...
    {
        Color c= TurtleSeward.getColor(penColor);
        if(c!=null)this.penColor=c;
        long timeStamp=storeCurrentState();
        return timeStamp;
    }
    ...
    {
        Color c= TurtleSeward.getColor(outlineColor);
    ...
    {
        Color c= TurtleSeward.getColor(fillColor);
    ...
    {
        Color c= TurtleSeward.getColor(color);
    ...
    private static boolean addMouseBinding(String methodName, TurtleSeward t, boolean append, boolean click, boolean repeat)
    {
        String className="";
        try
        {
    ...
    public static void zoomFit()
    {
        synchronized(turtleLock)
        {
            ...
            if(turtleStates.lastKey()>time && getStateSpeed(turtleStates.lastEntry().getValue())>0)
            {
                ...
                TurtleSeward t=getStateTurtle(turtleStates.lastEntry().getValue());
    ...
    private static void draw()
    {
        synchronized(turtleLock)
        {
            ...
            for (Map.Entry<Long, ArrayList> entry : turtleStates.tailMap(lastUpdate).headMap(renderTime).entrySet())
            {
                ...
                TurtleSeward t=getStateTurtle(entry.getValue());
            ...
            midscreen.drawImage(offscreenImage,0,0, null);
            TurtleSeward animatedTurtle=null;
            double percent=1;
            Long t2;
            t2=new Long(0);
    ...
    {
        TurtleSeward.width=width;
        TurtleSeward.height=height;
        TurtleSeward.setupBuffering();
        window.pack();
        updateAll();
    }
    ...
    private void processKeys()
    {
        ...
        for(String keyText:keysDownCopy)
        {
            if (keyBindings.containsKey(keyText))
            {
                ...
                {
                    TurtleSeward t=(TurtleSeward)binding.get(0);
